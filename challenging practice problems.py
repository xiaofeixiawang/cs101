...
def is_reciprocal(graph,page,node,k):
    if k<=0:
        if page==node:
            return True
        return False
    for pre in graph[page]:
        if is_reciprocal(graph,pre,node,k-1):
            return True
    return False
...
def helper(string,generation,dictionary):
    if generation==0:
        return string
    if len(string)==1:
        newstr=dictionary[3*string[0]]
        return helper(newstr,generation-1,dictionary)
    newstr=dictionary[string[len(string)-1]+string[0:2]]
    for i in range(1,len(string)-1):
        newstr+=dictionary[string[i-1:i+2]]
    newstr+=dictionary[string[len(string)-2:len(string)]+string[0]]
    return helper(newstr,generation-1,dictionary)

def cellular_automaton(string,pattern,generation):
    dictionary={'...':'.','..x':'.','.x.':'.','.xx':'.','x..':'.','x.x':'.','xx.':'.','xxx':'.'}
    if pattern>=128:
        dictionary['xxx']='x'
        pattern-=128
    if pattern>=64:
    ...
    if pattern>=1:
        dictionary['...']='x'
    return helper(string,generation,dictionary)
