...
def create_data_structure(string_input):
    ...
    while string_input.find('.')!=-1:
        ...
        if string_input.find('connected to ')!=-1 and string_input.find('connected to ')<string_input.find('.'):
            people=string_input[string_input.find(' to ')+4:string_input.find('.')].split(', ')
            network[name]=[people]
        if string_input.find(' play ')!=-1 and string_input.find(' play ')<string_input.find('.'):
            game=string_input[string_input.find(' play ')+6:string_input.find('.')].split(', ')
...
def add_connection(network, user_A, user_B):
    if not user_A in network or not user_B in network:
        return False
    List=get_connections(network, user_A)
    if not user_B in List:
        List.append(user_B)
    return network
...
def get_secondary_connections(network, user):
    ...
    if get_connections(network,user)==[]:
        return []
    for first in get_connections(network, user):
        for second in get_connections(network, first):
            if not second in res:
                res+=[second]
    return res
...
def count_common_connections(network, user_A, user_B):
    ...
    for people in get_connections(network, user_A):
        if people in get_connections(network, user_B):
            count+=1
    return count
